...
const ExpUpdate = () => {
    ...
    const {id} =  useParams()
    useEffect(()=>{
        axios.get('http://localhost:3002/Experience/'+id)
        .then(data=>{
            setComapny(data.data.comapny)
...
const HandleSubmit = (e) =>{
  ...
  axios.put('http://localhost:3002/Exp/update/'+id,formm)
  .then(result=>{
    console.log(result);
    navigate('/')
  ...
  return (
    ...
    <form className='flex flex-col items-start justify-center border-2 rounded-lg p-5 shadow-lg w-[50%]'>
        ...
        <input value={comapny} onChange={(e)=>setComapny(e.target.value)} className='border w-3/4 rounded-md p-1 text-sm text-gray-400' type="text" />
        <label  className='text-xl font-semibold' htmlFor="">Role</label>
        <input value={role} onChange={(e)=>setRole(e.target.value)} className='border w-3/4 rounded-md p-1 text-sm text-gray-400' type="text" />
        <label className='text-xl font-semibold' htmlFor="">Description</label>
        <textarea value={des} onChange={(e)=>setDes(e.target.value)} className='border p-1 text-sm text-gray-400 h-[200px] w-3/4' name="" id=""></textarea>
